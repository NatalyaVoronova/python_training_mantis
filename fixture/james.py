...
class JamesHelper:
    ...
    def ensure_user_exists(self, username, password):  # логин и пасворд пользователя
        james_config = self.app.config['james']
        session = JamesHelper.Session(
            james_config['host'], james_config['port'], james_config['username'], james_config['password'])  # 16'
    ...
    class Session:
        ...
        def __init__(self, host, port, username, password):  # логин и пасворд псервера
        ...
        def is_users_registred(self, username):
            self.write("verify %s\n" % username)
            res = self.telnet.expect([b"exists", b"does not exist"])  # b  - это использование байтовых строк
            return res[0] == 0
